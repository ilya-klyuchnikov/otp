...
i_let_2(Vs, As, E, Ctxt, Ren, Env, S) ->
    ...
    {Opnds, S1} = mapfoldl(fun (E1, S1) ->
                                   make_opnd(E1, ren__identity(), Env, S1)
                           end,
                           S, As),
...
i_clauses(Es, Cs, Ctxt, Ren, Env, S) ->
    ...
    {Ts, {Vs, Env0}} = mapfoldl(fun (E, {Vs, Env0}) ->
					{T, Vs1, Env1} =
					    make_template(E, Env0),
					{T, {Vs1 ++ Vs, Env1}}
				end,
				{[], Env}, Es),
    ...
    {Ren1, Env1, S1} =
	foldl(fun (V, {RenI, EnvI, SI}) ->
		      E = env__get(var_name(V), Env0),
		      {Opnd, S_1} = make_opnd(E, ren__identity(), EnvI,
					      SI),
    ...
    {Cs1, S2} = mapfoldl(fun (C, SI) ->
				 i_clause_head(C, Ts, Ren1, Env1, SI)
			 end,
			 S1, Cs),
...
i_clause_head(C, Ts, Ren, Env, S) ->
    ...
    {Ps1, S3} = mapfoldl(fun (P, SI) ->
				 i_pattern(P, Ren1, Env1, Ren, Env, SI)
			 end,
			 S2, Ps),
...
add_match_bindings(Bs, E) ->
	...
	false ->
	    Vs = [V || {V, EI} <- Bs, EI =/= any],
	    Es = [hd(get_ann(EI)) || {_V, EI} <- Bs, EI =/= any],
...
i_letrec(Es, B, Xs, Ctxt, Ren, Env, NoInline, S) ->
    ...
    {Es1, S5} = mapfoldl(fun (R, SI) ->
				 {E_1, S_1} = visit(R#ref.opnd, SI),
...
i_apply(E, Ctxt, Ren, Env, S) ->
    {Opnds, S1} = mapfoldl(fun (EI, SI) ->
                                   make_opnd(EI, Ren, Env, SI)
                           end,
                           S, apply_args(E)),
    ...
    case st__get_app_inlined(L, S3) of
        ...
        false ->
            ...
            %% application.
            {Es, S4} = mapfoldl(fun (Opnd, SI) ->
					visit_and_count_size(Opnd, SI)
				end,
				S3, Opnds),
            Arity = length(Es),
...
i_primop(E, Ren, Env, S) ->
    %% Visit the arguments for value.
    {As, S1} = mapfoldl(fun (EI, SI) ->
				i(EI, value, Ren, Env, SI)
			end,
			S, primop_args(E)),
...
i_binary(E, Ren, Env, S) ->
    ...
    {Es, S1} = mapfoldl(fun (EI, SI) ->
				i_bitstr(EI, Ren, Env, SI)
			end,
			S, binary_segments(E)),
...
i_map(E, Ctx, Ren, Env, S0) ->
    ...
    {Es, S2} = mapfoldl(fun (EI, SI) ->
		i_map_pair(EI, Ctx, Ren, Env, SI)
	end, S1, map_es(E)),
...
i_pattern(E, Ren, Env, Ren0, Env0, S) ->
	...
	map ->
	    {Es, S1} = mapfoldl(fun (EI, SI) ->
			i_map_pair_pattern(EI, Ren, Env, Ren0, Env0, SI)
		end, S, map_es(E)),
	...
	_ ->
		...
		true ->
                    {E, count_size(weight(literal), S)};
		false ->
		    {Es1, S1} = mapfoldl(fun (EI, SI) ->
						 i_pattern(EI, Ren, Env,
							   Ren0, Env0,
							   SI)
					 end,
					 S, data_es(E)),
...
bind_recursive(Vs, Opnds, Ren, Env, S) ->
    ...
    Fun = fun (R, EnvI) ->
		  Opnd = R#ref.opnd,
		  R#ref{opnd = Opnd#opnd{ren = Ren1, env = EnvI}}
	  end,
...
residualize_operands(Opnds, E, S) ->
    foldr(fun (Opnd, {EI, SI}) -> residualize_operand(Opnd, EI, SI) end,
          {E, S}, Opnds).
