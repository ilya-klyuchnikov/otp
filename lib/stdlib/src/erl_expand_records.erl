...
-spec guard_tests1(erl_parse:af_guard(), #exprec{}) -> {erl_parse:af_guard(), #exprec{}}.
...
expr({call,Anno,F,As0}, St0) ->
    % eqwalizer:ignore - F is not remote_call, - try infinite refimenement
...
-spec conj([record_access()], erl_parse:abstract_expr() | none) -> erl_parse:abstract_expr().
conj([], _E) ->
    % eqwalizer:ignore
    empty;
...
-spec lc_tq(erl_anno:anno(), erl_parse:af_qualifier_seq(), #exprec{}) -> {erl_parse:af_qualifier_seq(), #exprec{}}.
...
lc_tq(Anno, [{zip,AnnoG,G0} | Qs0], St0) ->
    ...
    % list of generators is translated into a list of generators,
    % but it's not expressible via specs
    % eqwalizer:ignore
...
-spec record_pattern(integer(), integer(), erl_parse:af_variable(), integer(), erl_anno:anno(), [erl_parse:af_atom() | erl_parse:af_variable()]) -> [erl_parse:af_atom() | erl_parse:af_variable()].
...
% eqwalizer:ignore `_` is a special hack in the AST/compiler
